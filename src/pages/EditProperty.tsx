...
const propertyTypes = ['Apartment', 'Studio', 'Shared Room', 'Private Room', 'House', 'Hostel'];
...
const EditProperty = () => {
  ...
  useEffect(() => {
    document.title = 'Edit Property | Dormify';
  }, []);
    ...
    const fetchProperty = async () => {
      try {
        ...
        setFormData({
          ...
          distance_to_university: data.distance_to_university || '',
          amenities: (data.amenities || []) as string[],
          landlord_id: data.landlord_id || user?.id || '',
        });
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="bedrooms" type="number" min="1" value={formData.bedrooms} onChange={(e) => handleInputChange('bedrooms', e.target.value)} required />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="bathrooms">Bathrooms</Label>
                  <Input id="bathrooms" type="number" min="1" value={formData.bathrooms} onChange={(e) => handleInputChange('bathrooms', e.target.value)} required />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="images" type="file" accept="image/*" multiple onChange={(e) => setImages(Array.from(e.target.files || []))} />
                {existingImages.length > 0 && (
                  <p className="text-sm text-gray-600">Existing images will be kept. New uploads will be added.</p>
                )}
              </div>
